...
async def status_task():
    while True:
        await client.cange_presence(game=discord.Game(name='People Type N!help', type = 3))
        await asyncio.sleep(5)
        await client.change_presence(game=discord.Game(name='over '+str(len(set(client.get_all_members())))+' users', type = 3))
        await asyncio.sleep(5)
        await client.change_presence(game=discord.Game(name='To'+str(len(client.servers))+' servers', type = 2))
        await asyncio.sleep(5)
...
async def help(ctx):
    ...
    embed.add_field(name = '``N!modhelp`` ',value ='Explaines all the commands which are only usable by Those who has moderation permissions. Like- Manage Nicknames, Manage Messagues, Kick/Ban Members,etc.',inline = False)
