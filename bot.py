         ...
         if reaction.emoji == '🇬':
          ...
          while True:
              ...
              if reaction.emoji == '🏵':
                    ...
                    embed.add_field(name = 'Setting up AutoPartner Channel(Admin Permission required)',value ='Using ``N!setuppartner`` command create a channel named thunder-partner and then you can use N!partner to partner with other servers.',inline = False)
                    ...
                    if reaction.emoji == ':musical_keyboard:':
                      embed.set_author(name='Music Help')
                    embed.set_image(url = 'https://image.ibb.co/caM2BK/help.gif')
                    embed.add_field(name = 'Plays A Song',value ='**__PLAY__**\n Plays A Song.Use it like N!Play url \n\n**__Join__**\nJoinS The Voice Channel.Use It like N!join channel \n\n**__Leave__**Leaves The Voice Channel N!leave \n\n**Pause**\nPauses The Voice Client N!pause \n\n**__Resume__** Resumes It again N!resume \n\n**__Queue__** Queue your song before playing N!queue',inline = False)

                    embed = discord.Embed(color = discord.Color((r << 16) + (g << 8) + b))
                    embed.set_author(name='Emoji Help *Removed*')
                    embed.set_image(url = 'https://image.ibb.co/caM2BK/help.gif')
...
async def help(ctx):
    if ctx.message.author.bot:
      ...
      embed.add_field(name = 'React with 🎦 ',value ='**REMOVED**List of Nitro emojis that you can use',inline = False)
      embed.add_field(name = 'React with :musical_keyboard:',value ='Music Commands available',inline = False)     
      dmmessage = await client.send_message(author,embed=embed)
      reaction1 = '🇲'
      reaction2 = '🇬'
      reaction3 = '🏵'
      reaction4 = '🎦'
      reaction5 = ':musical_keyboard: '
      await client.add_reaction(dmmessage, reaction1)
      await client.add_reaction(dmmessage, reaction2)
      await client.add_reaction(dmmessage, reaction3)
      await client.add_reaction(dmmessage, reaction4)
      await client.add_reaction(dmmessage, reaction5)
...
async def join(ctx):
    ...
    await client.say("I am finally in {channel}")
...
async def leave(ctx):
	...
	await client.say("I left The Chat Bye ;(") 
...
async def play(ctx, url):
	...
	player.start()
	await client.say("succesfuly playing {song}")
...
async def pause(ctx): 
    ...
    players [id].stop()
    await client.say("because of {user} your song is paused")
    
@client.command(pass_context=True)
async def resume(ctx):    
    id = ctx.message.server.id
    players [id].resume()
    await client.say("The song is started Thank {user}.")
...
async def queue(ctx, url):
	...
	voice_client = client.voice_channel_in(server)
	player = await voice_client.create_yt_dl_player(url)
	queus[server.id] = player
	await client.say("The queue thing now complete yay")
